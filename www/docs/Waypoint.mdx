...
A component (and related hook) provide a high level interface
...
"intersects" with the visable area of the scroll view. Waypoint will fire callback
...
By default waypoint observe scrolling on the device viewport, generally the `window`.
...
prop, which is passed to the underlying `IntersectionObserver`. As a covenience
...
before the are actually in view. Negative values shrink the viewport, and delays
...
The underlying hook powering the Waypoint component. Leaverage `useWaypoint`
